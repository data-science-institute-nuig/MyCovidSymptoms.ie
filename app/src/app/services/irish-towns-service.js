const irishTowns = {
  Mayo: [
    'Baile an Droichid (Ballindrait)',
    'Balla',
    'Ballina',
    'Ballindine',
    'Ballinrobe',
    'Ballycastle',
    'Ballyhaunis',
    'Bangor Erris',
    'Belmullet',
    'Belcarra',
    'Bellavary',
    'Bohola',
    'Bunnyconnelan West',
    'Castlebar',
    'Charlestown - Bellahy',
    'Claremorris',
    'Cong',
    'Crossmolina',
    'Doohooma (Dumha Thuama)',
    'Foxford',
    'Gob an Choire (Achill Sound)',
    'Irishtown',
    'Keel - Dooagh',
    'Kilkelly',
    'Killala',
    'Kilmaine',
    'Kiltimagh',
    'Knock',
    'Lahardane',
    'Louisburgh',
    'Mulranny',
    'Newport',
    'Shrule',
    'Swinford',
    'Turlough',
    'Westport',
  ],
  Donegal: [
    'Burtonport',
    'Anagaire (Annagry)',
    'An Bun Beag - Bunbeg - Derrybeg)',
    'An Charraig (Carrick)',
    'Dungloe',
    'Ardara',
    'Ballintra',
    'Ballybofey - Stranorlar',
    'Ballyliffin',
    'Ballyshannon',
    'Bridge End',
    'Buncrana',
    'Bundoran',
    'Bun na Leaca (Brinlack)',
    'Burnfoot',
    'Carndonagh',
    'Carraig Airt',
    'Carrigans',
    'Carrowkeel',
    'Castlefin',
    'Cill Charthaigh (Kilcar)',
    'Clonmany',
    'Cnoc na Muirleog (Crocknamurleog)',
    'Convoy',
    'Creeslough',
    'Culdaff',
    'Donegal',
    'Drumkeen',
    'Dunfanaghy',
    'Dunkineely',
    'Fahan',
    'Falcarragh',
    'Gleann Cholm Cille',
    'Gleneely',
    'Glenties',
    'Gort an Choirce (Gortahork)',
    'Greencastle',
    'Kildrum',
    'Killybegs',
    'Killygordan',
    'Kilmacrennan',
    'Laghy',
    'Letterkenny',
    'Lifford',
    'Loughanure',
    'Malin',
    'Manorcunningham',
    'Milford__1',
    'Meenlaragh',
    'Mountcharles',
    'Moville',
    'Muff',
    'Newtowncunningham',
    'Pettigo (Irish part)',
    'Quigleys Point',
    'Ramelton',
    'Rann na Feirste',
    'Raphoe',
    'Rathmullen',
    'St. Johnston',
    'Tievebane',
  ],
  Kildare: [
    'Allenwood',
    'Athgarvan',
    'Athy',
    'Ballymore Eustace',
    'Ballyoulster',
    'Ballyroe',
    'Ballysax',
    'Ballytore',
    'Blessington',
    'Brownstown',
    'Calverstown',
    'Carragh',
    'Castledermot',
    'Celbridge',
    'Clane',
    'Coill Dubh (Blackwood)',
    'Derrinturn',
    'Droichead Nua (Newbridge)',
    'Grangemore',
    'Johnstown',
    'Johnstownbridge',
    'Kilberry',
    'Kilcock',
    'Kilcullen',
    'Kildangan',
    'Kildare',
    'Kill',
    'Kilmeade (Kilmead)',
    'Kilmeage',
    'Kilteel',
    'Ladytown',
    'Leixlip Town',
    'Maddenstown',
    'Maynooth',
    'Milltown__2',
    'Monasterevin',
    'Naas',
    'Narraghmore',
    'Nurney',
    'Prosperous',
    'Rathangan',
    'Rathcoffey',
    'Robertstown',
    'Sallins',
    'Straffan',
    'Suncroft',
  ],
  'South Dublin': ['Brittas', 'Newcastle', 'Rathcoole__1', 'Saggart'],
  Leitrim: [
    'Ballinamore',
    'Carrigallen',
    'Dromahair',
    'Drumkeeran',
    'Drumod',
    'Drumshanbo',
    'Drumsna',
    'Keshcarrigan',
    'Kinlough',
    'Leitrim',
    'Manorhamilton',
    'Mohill',
    'Tullaghan',
  ],
  Wicklow: [
    'Arklow',
    'Ashford',
    'Aughrim',
    'Avoca',
    'Ballinaclash',
    'Ballyknockan',
    'Baltinglass',
    'Barndarrig',
    'Blessington',
    'Bray',
    'Carnew',
    'Coolboy',
    'Donard',
    'Dunlavin',
    'Enniskerry',
    'Glenealy',
    'Greystones - Delgany',
    'Kilcoole',
    'Kilmacanogue',
    'Kilpedder',
    'Knockananna',
    'Laragh',
    'Manor Kilbride',
    'Newcastle__2',
    'Newtownmountkennedy',
    'Rathdrum',
    'Rathnew',
    'Redcross',
    'Roundwood',
    'Shillelagh',
    'Stratford (Stratford-on-Slaney)',
    'Tinahely',
    'Wicklow',
  ],
  'Dublin 1': [
    'Abbey St',
    'Amiens St',
    'Capel St',
    'Dorset St',
    'Henry St',
    'Mountjoy Sq',
    'Marlboro St',
    'North Wall',
    "O'Connell St",
    'Parnell Sq',
    'Talbot St',
  ],
  'Dublin 2': [
    'Baggot St Upper, and Lower',
    'College Green',
    'Fitzwilliam Square',
    'Harcourt Street',
    'Kildare Street',
    'Lord Edward Street',
    'Merrion Square',
    'Mount Street, Upper, and Lower',
    'Nassau Street',
    'Pearse Street',
    'St. Stephen’s Green',
    'South Great Georges Street',
    'Leeson Street, Upper and Lower',
  ],
  'Dublin 3': [
    'Ballybough',
    'Cloniffe',
    'Clontarf',
    'Dollymount',
    'East Wall',
    'Fairview',
    'Marino',
  ],
  'Dublin 4': [
    'Ballsbridge',
    'Donnybrook',
    'Irishtown',
    'Merrion',
    'Pembroke',
    'Ringsend',
    'Sandymount',
  ],
  'Dublin 5': ['Artane', 'Harmonstown', 'Raheny'],
  'Dublin 6': [
    'Dartry',
    'Ranelagh',
    'Rathmines',
    'Rathgar',
    'Harold’s Cross',
    'Templeogue',
    'Terenure',
  ],
  'Dublin 7': ['Arbour Hill', 'Cabra', 'Phibsborough', 'Four Courts'],

  'Dublin 8': [
    'Dolphin’s Barn',
    'Inchicore',
    'Island Bridge',
    'Kilmainham',
    'Merchants Quay',
    'Portobello',
    'South Circular Road',
    'The Coombev',
  ],

  'Dublin 9': ['Beaumont', 'Drumcondra', 'Elm Mount', 'Griffith Avenue', 'Santry', 'Whitehall'],

  'Dublin 10': ['Ballyfermot'],

  'Dublin 11': [
    'Ballygal',
    'Cappagh',
    'Cremore',
    'Dubber',
    'Finglas',
    'Jamestown',
    'Kilshane',
    'Wadelai',
  ],

  'Dublin 12': ['Bluebell', 'Crumlin', 'Drimnagh', 'Walkinstown'],
  'Dublin 13': ['Baldoyle', 'Bayside', 'Donaghmede', 'Sutton', 'Howth'],
  'Dublin 14': ['Churchtown', 'Dundrum', 'Goatstown', 'Roebuck', 'Windy Arbour', 'Clonskeagh'],
  'Dublin 15': ['Blanchardstown', 'Castleknock', 'Clonee', 'Clonsilla', 'Corduff', 'Mulhuddart'],
  'Dublin 16': ['Ballinteer', 'Ballyboden', 'Knocklyon', 'Sandyford'],
  'Dublin 17': ['Belcamp', 'Balgrifin', 'Clonshaugh', 'Priorswood', 'Darndale', 'Riverside'],
  'Dublin 18': [
    'Cabinteely',
    'Carrickmines',
    'Foxrock',
    'Kilternan',
    'Sandyford',
    'Ticknock',
    'Ballyedmonduff',
    'Stepaside',
    'Leopardstown',
  ],
  'Dublin 20': ['Chapelizod', 'Palmerstown'],
  'Dublin 22': ['Bawnogue', 'Clondalkin', 'Neilstown'],
  'Dublin 24': ['Firhouse', 'Jobstown', 'Kilnamanagh', 'Oldbawn', 'Tallaght'],
  Fingal: [
    'Balbriggan',
    'Ballyboghil',
    'Balrothery',
    'Donabate',
    'Garristown',
    'Kinsaley',
    'Kinsealy - Drinan',
    'Loughshinny',
    'Lusk',
    'Malahide',
    'Naul',
    'Oldtown',
    'Portmarnock',
    'Portrane',
    'Rivermeade',
    'Rush',
    'Skerries',
    'Swords',
  ],
  Cavan: [
    'Arva',
    'Bailieborough',
    'Ballinagh',
    'Ballyconnell',
    'Ballyhaise',
    'Ballyjamesduff',
    'Belturbet',
    'Blacklion',
    'Butlersbridge',
    'Cavan',
    'Cootehill',
    'Killeshandra',
    'Kilnaleck',
    'Kingscourt',
    'Lough Gowna',
    'Mullagh',
    'Shercock',
    'Swanlinbar',
    'Virginia',
  ],
  Kilkenny: [
    'Ballyhale',
    'Ballyragget',
    'Bennettsbridge',
    'Callan',
    'Castlecomer - Donaguile',
    'Clogh - Chatsworth',
    'Fiddown',
    'Freshford',
    'Goresbridge',
    'Gowran',
    'Graiguenamanagh - Tinnahinch',
    'Inistioge',
    'Johnstown__1',
    'Kells',
    'Kilkenny',
    'Kilmacow',
    'Kilmoganny',
    'Knocktopher',
    'Lawcus - Stoneyford',
    'Moneenroe',
    'Mooncoin',
    'Mullinavat',
    'New Ross',
    'Paulstown',
    'Piltown',
    'Slieverue',
    'Thomastown',
    'Urlingford',
    'Waterford',
  ],
  Clare: [
    'Ardnacrusha (Castlebank)',
    'Ardskeagh',
    'Ballycannan',
    'Ballyvaughan',
    'Bunratty',
    'Clonlara',
    'Corofin',
    'Cratloe',
    'Crusheen',
    'Doonbeg',
    'Ennis',
    'Ennistymon',
    'Feakle',
    'Inagh',
    'Kildysart',
    'Kilfenora',
    'Kilkee',
    'Kilkishen',
    'Killaloe',
    'Kilmaley',
    'Kilmihill',
    'Kilmurry (Ballymulcashel)',
    'Kilrush',
    'Lahinch',
    'Limerick',
    'Liscannor',
    'Lisdoonvarna',
    'Lissycasey',
    'Miltown Malbay',
    'Mountshannon',
    'Newmarket-on-Fergus',
    "O'Briensbridge - Montpelier",
    'Quilty',
    'Quin',
    'Scarriff - Tuamgraney',
    'Sixmilebridge',
    'Shannon Town',
    'Tulla',
    'Whitegate',
  ],
  Monaghan: [
    'Annayalla (Annyalla)',
    'Ballybay',
    'Bellanode',
    'Cabragh (Threemilehouse)',
    'Carrickmacross',
    'Castleblayney',
    'Clones',
    'Clontibret',
    'Doohamlat (Doohamlet)',
    'Emyvale',
    'Glasslough',
    'Inniskeen',
    'Monaghan',
    'Newbliss',
    'Oram',
    'Rockcorry',
    'Scotshouse',
    'Scotstown',
    'Smithborough',
  ],
  Tipperary: [
    'Ardfinnan',
    'Ballina',
    'Ballingarry__1',
    'Ballyclerahan',
    'Ballynonty',
    'Ballyporeen',
    'Bansha',
    'Birr',
    'Boherlahan',
    'Borrisokane',
    'Borrisoleigh',
    'Cahir',
    'Cappawhite',
    'Carrick-on-Suir',
    'Cashel',
    'Clogheen',
    'Clonmel',
    'Cloughjordan',
    'Donaskeigh',
    'Drangan',
    'Dromineer',
    'Dundrum',
    'Emly',
    'Fethard',
    'Golden',
    'Gortnahoo',
    'Holycross',
    'Killenaule',
    'Kilsheelan',
    'Lisronagh',
    'Littleton',
    'Monard',
    'Mullinahone',
    'Nenagh',
    'Newcastle__1',
    'Newport__1',
    'Newtown__1',
    'Portroe',
    'Puckaun',
    'Reardnogy',
    'Roscrea',
    'Rosegreen',
    'Silvermines',
    'Templemore',
    'Templetuohy',
    'The Commons',
    'Thurles',
    'Tipperary',
    'Toomevara',
    'Twomileborris',
  ],
  Waterford: [
    'Aglish',
    'An Rinn (Ring)',
    'Ardmore',
    'Ballinroad',
    'Ballymacarbry',
    'Cappoquin',
    'Cheekpoint',
    'Clashmore',
    'Clonmel',
    'Dungarvan',
    'Dunhill',
    'Dunmore East',
    'Kill__1',
    'Kilmacthomas',
    'Kilmeaden',
    'Lemybrien',
    'Lismore',
    'Passage East',
    'Portlaw',
    'Stradbally__1',
    'Tallow',
    'Tramore',
    'Villierstown',
    'Waterford',
  ],
  Cork: [
    'Aghada - Farsid - Rostellan',
    'Ballymakeery',
    'Ballineen - Enniskean',
    'Ballinspittle',
    'Ballyclogh',
    'Ballycotton',
    'Ballydehob',
    'Ballydesmond',
    'Ballygarvan',
    'Ballyhooly',
    'Ballymacoda',
    'Ballynoe',
    'Baltimore',
    'Bandon',
    'Banteer',
    'Bantry Town',
    'Ballingeary',
    'Belgooly',
    'Blarney',
    'Boherbue',
    'Bridebridge',
    'Buttevant',
    'Bweeng',
    'Carrigaline',
    'Carrignavar',
    'Carrigtwohill',
    'Castlelyons',
    'Castlemagner',
    'Castlemartyr',
    'Castletownbere',
    'Castletownroche',
    'Castletownsend',
    'Churchtown',
    'Clonakilty',
    'Clondrohid',
    'Clondulane',
    'Cloughduv',
    'Cloyne',
    'Coachford',
    'Cobh',
    'Conna',
    'Cork City',
    'Courtbrack',
    'Courtmacsherry',
    'Crookstown',
    'Crossbarry',
    'Crosshaven - Churchbay',
    'Doneraile',
    'Drimoleague',
    'Dromina',
    'Drommahane',
    'Dunderrow',
    'Dunmanway',
    'Durrus',
    'Farran',
    'Fermoy',
    'Fountainstown',
    'Freemount',
    'Glanworth',
    'Glengarriff',
    'Glenville',
    'Grenagh',
    'Halfway',
    'Inchigeelagh',
    'Innishannon',
    'Kanturk',
    'Kilbrin',
    'Kilbrittain',
    'Kilcrohane',
    'Kildorrery',
    'Killavullen',
    'Killeagh',
    'Killumney',
    'Kilworth',
    'Kinsale',
    'Knockglass',
    'Knocknagree',
    'Knockraha',
    'Leap',
    'Liscarroll',
    'Lyre',
    'Macroom',
    'Mallow',
    'Midleton',
    'Milford',
    'Millstreet',
    'Mitchelstown',
    'Model Village (Dripsey)',
    'Mogeely',
    'Newmarket',
    'Newtown (Newtownshandrum)',
    'New Twopothouse',
    'Passage West',
    'Rathard - Aherla',
    'Rathcoole',
    'Rathcormac',
    'Rathluirc (Charleville)',
    'Ringaskiddy - Loughbeg',
    'Riverstick',
    'Rosscarbery',
    'Saleen',
    'Schull',
    'Shanagarry',
    'Shanbally',
    'Shanballymore',
    'Skibbereen',
    'Timoleague',
    'Tower',
    'Unionhall',
    'Watergrasshill',
    'Whitechurch',
    'Whitegate__1',
    'Youghal',
  ],
  Galway: [
    'Galway City',
    'Abbeyknockmoy',
    'Ahascragh',
    'Carraroe',
    'Annaghdown',
    'Spiddal',
    'Athenry',
    'Claregalway',
    'Cong',
    'Ballinasloe',
    'Ballinderreen',
    'Ballygar',
    'Bearna',
    'Kilrona)',
    'Clarinbridge',
    'Clifden',
    'Cloonboo',
    'Corrandulla',
    'Corrofin',
    'Craughwell',
    'Dunmore',
    'Eyrecourt',
    'Glenamaddy',
    'Gort',
    'Headford',
    'Kilcolgan',
    'Killimor',
    'Kinvara',
    'Lackaghbeg',
    'Loughrea Town',
    'Maigh Cuilinn (Moycullen)',
    'Milltown',
    'Monivea',
    'Mount Bellew',
    'Moylough',
    'Na Forbacha (Furbogh)',
    'Oranmore',
    'Oughterard',
    'Portumna',
    'Rosscahill',
    'Roundstone',
    'Tuam',
    'Turloughmore',
    'Williamstown',
    'Woodford',
  ],
  Wexford: [
    'Adamstown',
    'Arthurstown',
    'Ballycanew',
    'Ballycullane',
    'Ballyedmond',
    'Ballygeary (Rosslare Harbour)',
    'Ballyhack',
    'Ballyhogue',
    'Ballymurn',
    'Ballysimon',
    'Barntown',
    'Bree',
    'Bridgetown',
    'Bunclody - Carrickduff',
    'Camolin',
    'Campile',
    'Castlebridge - Blackwater',
    'Castle Ellis',
    'Clonegal',
    'Clongeen',
    'Clonroche',
    'Coolgreany',
    'Courtown Harbour - Riverchapel - Ardamine',
    'Danescastle',
    'Duncannon',
    'Duncormick',
    'Enniscorthy',
    'Ferns',
    'Fethard-on-Sea',
    'Gorey',
    'Grahormac',
    'Killinierin',
    'Killurin__1',
    'Kilmore',
    'Kilmore Quay',
    'Kilmuckeridge (Ford)',
    'Kilmyshall',
    'Kilrane',
    'Murntown',
    'Newbawn',
    'New Ross',
    'Oilgate (Oylegate)',
    'Oulart',
    'Piercetown',
    'Rosslare',
    'Taghmon',
    'The Ballagh',
    'Wexford',
  ],
  Carlow: [
    'Ballinabrannagh',
    'Ballon',
    'Borris',
    'Bunclody - Carrickduff',
    'Clonegal',
    'Fennagh',
    'Graiguenamanagh - Tinnahinch',
    'Hacketstown',
    'Kernanstown',
    'Kildavin',
    'Leighlinbridge',
    'Muinebeag (Bagenalstown)',
    'Myshall',
    'Palatine',
    'Rathtoe',
    'Rathvilly',
    'Tinriland (Tinryland)',
    'Tullow',
  ],
  Kerry: [
    'Abbeydorney',
    'An Daingean (Dingle)',
    'Annascaul',
    'Ardfert',
    'Ballybunion',
    'Ballyduff',
    'Ballyheigue',
    'Ballylongford',
    'Beaufort',
    'Brosna',
    'Cahirciveen (Cahersiveen)',
    'Castlegregory',
    'Castleisland',
    'Castlemaine',
    'Causeway',
    'Cromane',
    'Duagh',
    'Fenit',
    'Fieries',
    'Glenbeigh',
    'Gneevgullia',
    'Kenmare',
    'Kilcummin',
    'Kilflynn',
    'Kilgarvan',
    'Killarney',
    'Killorglin',
    'Knightstown',
    'Listowel',
    'Lixnaw',
    'Milltown__1',
    'Newtownsandes',
    'Portmagee',
    'Rathmore',
    'Sneem',
    'Spa',
    'Tarbert',
    'Tralee',
    'Waterville - Spunkane',
  ],
  Meath: [
    'Ashbourne',
    'Athboy',
    'Balbradagh',
    'Ballivor',
    'Carlanstown',
    'Clonard',
    'Clonee Village',
    'Crossakeel',
    'Donore',
    'Drogheda',
    'Drumconrath',
    'Duleek',
    'Dunboyne',
    'Dunderry',
    'Dunshaughlin',
    'Enfield',
    'Gormanston',
    'Julianstown',
    'Kells',
    'Kentstown',
    'Kilcloon',
    'Kildalkey',
    'Kilmainham',
    'Kilmessan',
    'Laytown - Bettystown - Mornington - Donacarney',
    'Longwood',
    'Navan (An Uaimh)',
    'Nobber',
    'Oldcastle',
    'Rathmolyon',
    'Ratoath',
    'Slane',
    'Stamullen',
    'Summerhill',
    'Trim',
  ],
  Leitrim: ['Carrick-on-Shannon', 'Roosky'],
  Westmeath: [
    'Athlone',
    'Ballinalack',
    'Ballymore',
    'Ballynacargy',
    'Baylin',
    'Castlepollard',
    'Castletown Geoghegan',
    'Clonmellon',
    'Collinstown',
    'Delvin',
    'Glassan',
    'Kilbeggan',
    'Killucan - Rathwire',
    'Kinnegad',
    'Moate',
    'Mullingar',
    'Multyfarnham',
    'Raharney',
    'Rathowen',
    'Rochfortbridge',
    'Tyrrellspass',
  ],
  Offaly: [
    'Ballinagar',
    'Ballycumber',
    'Banagher',
    'Belmont',
    'Birr',
    'Bracknagh',
    'Clara',
    'Cloghan',
    'Clonbullogue',
    'Clonygowan',
    'Coolagary (Walsh Island)',
    'Crinkle (Crinkill)',
    'Daingean',
    'Edenderry',
    'Ferbane',
    'Geashill',
    'Kilcormac (Frankford)',
    'Killeigh',
    'Killurin',
    'Kinnitty',
    'Moneygall',
    'Mucklagh',
    'Pollagh',
    'Portarlington',
    'Rhode',
    'Shannonbridge',
    'Shinrone',
    'Tullamore',
  ],
  Sligo: [
    'Ballinacarrow',
    'Ballincar',
    'Ballintogher',
    'Ballisodare (Ballysadare)',
    'Ballygawley',
    'Ballymote',
    'Carney',
    'Charlestown - Bellahy',
    'Cliffoney',
    'Collooney',
    'Coolaney',
    'Dromore West',
    'Easky',
    'Grange',
    'Gurteen',
    'Inniscrone (Enniscrone)',
    'Mullaghmore',
    'Riverstown',
    'Rosses Point',
    'Sligo',
    'Strandhill',
    'Tubbercurry',
  ],
  Laois: [
    'Abbeyleix',
    'Ballinakill',
    'Ballybrittas',
    'Ballycolla',
    'Ballylinan',
    'Ballyroan',
    'Borris-in-Ossory',
    'Castletown',
    'Clonaslee',
    'Durrow',
    'Emo',
    'Erril',
    'Killenard',
    'Mountmellick',
    'Mountrath',
    'Newtown',
    'Portarlington',
    'Portlaoise (Maryborough)',
    'Rathdowney',
    'Stradbally',
    'The Swan',
  ],
  Louth: [
    'Annagassan',
    'Ardee',
    'Baltray',
    'Beaulieu',
    'Carlingford',
    'Castlebellingham - Kilsaran',
    'Clogherhead',
    'Collon',
    'Drogheda',
    'Dromiskin',
    'Dundalk',
    'Dunleer',
    'Jenkinstown',
    'Knockbridge',
    'Lordship',
    'Louth',
    'Omeath',
    'Tallanstown',
    'Termonfeckin',
    'Tinure',
    'Tullyallen',
  ],
  Longford: [
    'Aughnacliffe',
    'Ballinalee',
    'Ballymahon',
    'Drumlish',
    'Granard Town',
    'Keenagh',
    'Killashee',
    'Lanesborough - Ballyleague',
    'Legan',
    'Longford',
    'Meathas Truim (Edgeworthstown)',
    'Newtownforbes',
  ],
  Limerick: [
    'Abbeyfeale',
    'Adare',
    'Annacotty',
    'Ardagh',
    'Askeaton',
    'Athea',
    'Ballingarry',
    'Ballyagran',
    'Ballyhahill',
    'Ballylanders',
    'Broadford',
    'Bruff',
    'Bruree',
    'Caherconlish',
    'Cappamore',
    'Carrigkerry',
    'Castleconnell',
    'Clarina',
    'Croagh',
    'Croom',
    'Doon',
    'Drumcollogher',
    'Fedamore',
    'Foynes',
    'Galbally',
    'Glen',
    'Glin',
    'Hospital',
    'Kildimo',
    'Kilfinane',
    'Kilmallock',
    'Kilteely',
    'Knocklong',
    'Limerick',
    'Moroe',
    'Mountcollins',
    'Mungret',
    'Newcastle West',
    "O'Briensbridge - Montpelier",
    'Oola',
    'Pallas Grean',
    'Pallaskenry',
    'Patrickswell',
    'Rathkeale',
    'Shanagolden',
    'Toornafulla',
  ],
  Roscommon: [
    'Athlone',
    'Athleague',
    'Ballaghaderreen',
    'Ballinameen',
    'Ballinasloe',
    'Ballinlough',
    'Ballintober',
    'Ballyfarnan',
    'Ballyforan',
    'Bellanagare',
    'Boyle',
    'Carrick-on-Shannon',
    'Castlerea',
    'Clontuskert',
    'Cloonfad',
    'Cootehall',
    'Elphin',
    'Frenchpark',
    'Keadew',
    'Knockcroghery',
    'Lanesborough - Ballyleague',
    'Loughglinn',
    'Roosky',
    'Roscommon',
    'Strokestown',
    'Termonbarry (Tarmonbarry)',
    'Tulsk',
  ],
};

export class IrishTownsService {
  // As per https://kuntaliitto.fi/sites/default/files/media/file/Ervat_Sairaanhoitopiirit2019.pdf
  static getCounties() {
    return [
      'Carlow',
      'Cavan',
      'Clare',
      'Cork',
      'Donegal',
      'Dublin 1',
      'Dublin 10',
      'Dublin 11',
      'Dublin 12',
      'Dublin 13',
      'Dublin 14',
      'Dublin 15',
      'Dublin 16',
      'Dublin 17',
      'Dublin 18',
      'Dublin 2',
      'Dublin 20',
      'Dublin 22',
      'Dublin 24',
      'Dublin 3',
      'Dublin 4',
      'Dublin 5',
      'Dublin 6',
      'Dublin 7',
      'Dublin 8',
      'Dublin 9',
      'Fingal',
      'Galway',
      'Kerry',
      'Kildare',
      'Kilkenny',
      'Laois',
      'Leitrim',
      'Limerick',
      'Longford',
      'Louth',
      'Mayo',
      'Meath',
      'Monaghan',
      'Offaly',
      'Roscommon',
      'Sligo',
      'South Dublin',
      'Tipperary',
      'Waterford',
      'Westmeath',
      'Wexford',
      'Wicklow',
    ];
  }

  static getTowns(county) {
    return irishTowns[county];
  }
}
